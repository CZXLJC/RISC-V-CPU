...
module divclk(
    ...
    output logic clk_16x,     // 16倍的波特率采样时钟 153600Hz (9.6KHz*16)
    output logic clk_x,       // 9600Hz 波特率时钟
    output logic clk_ms, // 1KHz 时钟信号
    output logic clk_500ms,    // 500ms 时钟信号
    output logic clk_1s        // 1s 时钟信号
);

    // 参数定义
    parameter int CNT_MS      = 50000;   // 100M/1000 = 100kHz (生成1KHz信号)
    parameter int CNT_16X      = 326;     // 100M/153600, 产生 clk_16x
    parameter int CNT_16X_TO_X = 16; // 16倍分频
    parameter int CNT_MS_TO_500MS = 500; // 1KHz * 500ms = 500次
    parameter int CNT_1S     = 1000;    // 1KHz * 1s = 1000次
