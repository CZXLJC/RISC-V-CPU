...
module Stage_ID(
    ...
    output logic [2:0]  BLUControl,   // 分支控制信号
    ...
    assign rd     = inst[11:7];
    assign BLUControl = funct3; // 分支控制信号
    ...
    // 分支目标计算（示例：BEQ指令）
    //-----------------------------
    assign BranchTarget = pc_plus4 + imm32 - 32'h4; // PC+4 + 偏移量
    assign BranchTaken = Branch && (rdata1 == rdata2); // 分支条件成立
