...
module ForwardingUnit(
    ...
    always_comb begin
        ...
        if (RegWrite_ex_mem && (rd_ex_mem != 0) && (rd_ex_mem == rs1_id)) 
            ForwardA = 2'b01;  // 前递EX/MEM阶段的结果
        else if (RegWrite_mem_wb && (rd_mem_wb != 0) && (rd_mem_wb == rs1_id)) 
        ...
        // ForwardB逻辑（rs2前递）
        if (RegWrite_ex_mem && (rd_ex_mem != 0) && (rd_ex_mem == rs2_id)) 
            ForwardB = 2'b01;
        else if (RegWrite_mem_wb && (rd_mem_wb != 0) && (rd_mem_wb == rs2_id)) 
            ForwardB = 2'b10;
        else 
            ForwardB = 2'b00;
    end
