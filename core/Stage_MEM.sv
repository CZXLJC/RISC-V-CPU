...
module Stage_MEM(
    ...
    // 数据信号
    input  logic [`DATA_WID] ALUResult,  // 内存地址（来自ALU结果）
    input  logic [`DATA_WID] rdata2,      // 存储数据（来自寄存器）

    // 前递信号
    input  logic Forward_wb, // 前递控制信号，表示是否需要前递数据
    input  logic [`DATA_WID] Forward_data_wb, // 前递数据A
    // 数据存储器接口
    output logic [`DATA_WID] mem_addr,    // 内存地址
    output logic        mem_we,      // 写使能
    output logic [`DATA_WID] mem_wdata,   // 写入数据
    input  logic [`DATA_WID] mem_rdata    // 读取数据（来自DataMem）
);
    // 直接连接信号
