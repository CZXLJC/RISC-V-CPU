...
module IF_ID(
    ...
    input  logic [`DATA_WID] inst_in,      // 来自IF阶段的指令
    // input  logic [`DATA_WID] pc_plus4_in,  // 来自IF阶段的PC+4
    input  logic [`DATA_WID] pc_curr_in, 
    output logic [`DATA_WID] inst_out,     // 输出到ID阶段的指令
    // output logic [`DATA_WID] pc_plus4_out  // 输出到ID阶段的PC+4
    output logic [`DATA_WID] pc_curr_out   // 输出到ID阶段的PC+4
);
